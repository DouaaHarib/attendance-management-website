...
if(isset($_GET['id'])) {
    ...
    foreach($EST->filieres->filiere as $filiere){
        if($filiere['idfiliere']==$id){
            $idFiliere=$filiere['idFiliere'];
            $nomFiliere=$filiere->nom;
            $Dept=$filiere->departement['id'];
            $chefFiliere=$filiere->chefFiliere['matricule'];
...
if(isset($_GET['modifier'])) {
    $newID=$_GET['idFiliere'];
    $newName=$_GET['nomFiliere'];
    $Dept=$_GET['Dept'];
    $newChefFiliere=$_GET['chefFiliere'];
    
    foreach($EST->filieres->filiere as $filiere){
		if($filiere['idfiliere']== $newID){
            // $departement['idDept']= $newID;
			$filiere->departement['id'] = $Dept;
            $filiere->chefFiliere['matricule'] = $newChefFiliere;
            $filiere->nom = $newName;
			break;
		}}
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Modifier Filiere </h6>
    </div>
    <div class="card-body">
        ...
        <form action="" method="get">
                ...
                <input type="text" value="<?php echo $id?>" name="idFiliere" class="form-control" id="idDept" >
                </div>
                <div class="mb-3">
                <label for="nomDept" class="col-form-label">Nom filiere:</label>
                <input type="text" value="<?php echo $nomFiliere?>" class="form-control" name="nomFiliere" id="nomDept"></input>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" name="Dept" aria-label="Default select example">
                        <option  selected><?php foreach($EST->departements->departement as $departement) { 
                            if($departement['idDept']=="$Dept"){echo $departement->nom ; break; }
                        }?>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" name="chefFiliere" aria-label="Default select example">
                        <option  selected><?php foreach($EST->users->user as $user) { 
                            if($user['matricule']=="$chefFiliere"){echo $user->nom." ".$user->prenom ; break; }
                        }?>
            ...
            <a href="filiere.php"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">cancel</button></a>
